...
Sent from my iPhone!'''
    assert ('Hey!', 'Sent from my iPhone!') == bruteforce.extract_signature(msg_body)


def test_mailbox_for_iphone_signature():
    msg_body = """Blah
Sent from Mailbox for iPhone"""
    assert ("Blah", "Sent from Mailbox for iPhone") == bruteforce.extract_signature(msg_body)
...
def test_get_signature_candidate():
    ...
    with patch.object(bruteforce, 'SIGNATURE_MAX_LINES', 2):
        ...
        assert ['signature1', '', 'signature2'] == bruteforce.get_signature_candidate(lines)
